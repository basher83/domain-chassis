...
**Agent SDK projects require execution environment tagging.** When a gate targets a project built on the Agent SDK (or any tool that spawns Claude Code as a subprocess), operational checkpoints requiring live SDK execution cannot be completed from within a Claude Code session (nested session constraint). The gate must tag these checkpoints as `[operator-terminal]` and separate them from checkpoints that an agent can verify via unit tests or structural inspection. This prevents an executing agent from discovering the constraint mid-execution after completing other checkpoints. The gate should also document the execution commands for operator-terminal checkpoints inline, since the operator runs them from a separate terminal.
